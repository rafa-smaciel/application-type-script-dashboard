...
interface IRouteParams {
    match: {
        params: {
            // movimentType: string; //É um type, pois no arquivos de rotas também coloquei type.
...
const List: React.FC<IRouteParams> = ({ match }) => { //No caso do return, dentro do useMemo, eu preciso pegar o tipo do URL, no caso desse tipo ele vem dos parametros da rota. Como ele vem dos parametros da rota, agente pode vir aqui e colocar como parametro da nossa lista, um cara chamado match, esse match esta disponivel graças ao react router dom. Porque o nosso lista vem dentro do Route path, do arquivo Routes. Esse cara esta dentro de um switch e dentro de um layout, que devolve o nosso AppRoutes  para  nosso index, que esta dento do browserRouter. o browser router disponibiliza pra gente esse match, que ele vai conter os parametros que tem na rota "URL". Porem, para ser usada, é necessário criar uma interface.
    ...
    const [monthSelected, setMonthSelected] = useState<number>(new Date().getMonth() + 1);//Passo 29, passo de string para number. Nesse mesmo passo, necessário criar a const handleMonthSelected  //Nesse estado eu guardo o mes selecionado no filtro. //(String(new Date().getMonth() +1)), serve para que toda a vez que iniciarmos a lista ele mostre o mês atual.
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [yearSelected, setYearSelected] = useState<number>(new Date().getFullYear()); //Passo 29, passo de string para number. Nesse mesmo passo, necessário criar a const handleYearSelected //Nesse estado eu guardo o ano selecionado no filtro. //(String(new Date().getFullYear())), serve para que toda a vez que iniciarmos a lista ele mostre o ano atual.

    const [frequencyFilterSelected, setFrequencyFilterSelected] = useState(['recorrente', 'eventual']); //Para o useEffect do handleFrequency, deve-se criar um novo estado para armazenar a frequencia. Portanto; eu estou dizendo que o nosso estado, selectedFrequecy, ele vai começar com um verto, um array, com os dois filtros de frequencia habilitados. Recorrente e Eventual.
    
    const movimentType = match.params.type;

    //Codigo refatorado e entrou a const pageData no luga. Passo 28.
    ...
    //     return movimentType === 'entry-balance' ? 'Entradas' : 'Saídas'//Operador Ternário
    // },[movimentType]); //Se eu colocar colchetes, sem nada, ele executa uma unica vez. Se eu passar o type no caso, eu estou criando uma dependencia.


    // const lineColor = useMemo(() => { //Não esta funcionando 19/02/2022 - Rafael
    //     return movimentType === 'entry-balance' ? '#F7931B' : '#E44C4E' //Operador Ternário
    // },[movimentType]); //Agora vamos mudar a cor do subinhado a medida que eu mudar as telas.
    ...
    //     return movimentType === 'entry-balance' ? gains : expenses; //Operador Ternário
    // },[movimentType]);
    ...
    const handleFrequencyClick = (frequency: string) => {
        const alreadySelected = frequencyFilterSelected.findIndex(item => item === frequency) //Eu quero saber se quando o usuario clicar em uma determinado estado, se ele ja esta selecionado ou não. Ou seja, explicando a const. Quando o usuario clicar, por exemplo em recorrentes, dentro dos parenteses estará o recorrente; eu quero encontrar o index em que o item seja igual a frequencia. 
        if(alreadySelected >= 0){
            const filtered = frequencyFilterSelected.filter(item => item !== frequency);//Para executar a funcionalidade de filtro do recorrente ou eventual, eu vou criar uma constante que eu vu chamar de filtered que será igual ao selectedFrequency, e depois eu vou filtra, por isso ".filter", ou seja, irei filtrar, fazer um filtro. Dentro desse filtro, eu vou percorrer as frequencias que vou chamar de item; portanto, em sintexe eu vou pegar minha coleção que tera todo os filtro la e ai eu estou filtrando ela. Eu vou pedir para ela retornar todos os filtros que estam guardados la, menos esse que existe; ou seja, se o usuario clicar, se o filtro ja existir, é pq ele ta quenrendo desmarcar ele. 
            setFrequencyFilterSelected(filtered); //Eu mando la pra dentro somente o que o usuario escolheu manter filtrado.
        }else{
            setFrequencyFilterSelected((prev) => [...prev, frequency]); //Entre colchetes para entender que é uma lista. Continuando a lógica acima, com o else; caso o filtro ele não existe ainda é pq o usuario ainda esta clicando nele para que de fato ele seja filtrado. Inserindo uma prev com arrow funtion, eu consigo recuperar o estado anterior, para manter ele; então vou pegar o estado anterior, vou manter o que estiver selecionado la; então eu estou pegando do estado anterior "prev" e espalhando aqui dentro através do "...prev" e junto com os filtros anteriores eu quero adicionar o filtro frequency que o usuario esta adicionando.
        }
    }

    const handleMonthSelected = (month: string) => { //Passo 29.
        try {
    ...
    const handleYearSelected = (year: string) => { // Passo 29.
        try {
    ...
    useEffect(() => {
        ...
        const filteredData = data.filter(item => { //Vou fazer um map no listData. E para cada item eu vou devolver com o return e vou copiar o formato que ele esta esperando do interface, conforme abaixo:
            ...
            const month = date.getMonth() + 1; //Passo 29, retiro a converção para string //Para facilitar, vou deixar separado; //Para igualar com o retorno da função month === monthSelected, vou converter esse dado em string;
            const year = date.getFullYear(); //Passo 29, retiro a converção para string //Para igualar com o retorno da função year === yearSelected, vou converter esse dado em string;

            return month === monthSelected && year === yearSelected && frequencyFilterSelected.includes(item.frequency); //ou seja, eu vou retornar as tarefas que possuem o mês igual ao mês slecionado, e o ano selecionado.
    ...
    },[pageData, monthSelected, yearSelected, data.length, frequencyFilterSelected]);//Se não colocarmos nada nos colchetes, ele é disparado somente 1 vez. Se colocarmos uma constante ou variavel nos colchetes, ele sempre vai recarregar/renderizar se o valor que agente colocar nos colchetes mudar.
